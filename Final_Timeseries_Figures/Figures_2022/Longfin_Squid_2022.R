...
ds<-read.csv("longfin_spring_2022.csv", header = TRUE)

# Creat a GAM - adjust k and remember to check model
mod<- gam(strat.biomass ~ s(YEAR, k=15), data = ds)
summary(mod) #check out model
gam.check(mod)

pdata <- with(ds, data.frame(YEAR = YEAR))
p2_mod <- predict(mod, newdata = pdata,  type = "terms", se.fit = TRUE)
intercept = 2.93212  # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[54:55, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'Longfin Squid') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("longfin_fall_2022.csv", header = TRUE)
...
# Creat a GAM - adjust k and remember to check model
mod2<- gam(strat.biomass ~ s(YEAR, k=15), data = ds2)
...
intercept2 = 13.56961  # look at p2_mod and extract the intercept
pdata2 <- transform(pdata2, p2_mod2 = p2_mod2$fit[,1], se2 = p2_mod2$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod2.d <- Deriv(mod2, n=56) # n is the number of YEARs
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2[56, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
