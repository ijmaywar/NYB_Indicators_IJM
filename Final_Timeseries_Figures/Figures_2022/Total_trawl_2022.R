## Figures for Total trawl biomass
...
ds<-read.csv("trawl_spring_2022.csv", header = TRUE)
...
intercept = 190.1859   # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'Total Trawl Biomass') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("trawl_fall_2022.csv", header = TRUE)
...
intercept2 = 127.6685     # look at p2_mod and extract the intercept
