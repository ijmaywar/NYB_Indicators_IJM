...
ds<-read.csv("forage_spring_2022.csv", header = TRUE)
...
intercept = 23.24884  # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'Forage Species') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("forage_fall_2022.csv", header = TRUE)
...
intercept2 = 26.08335    # look at p2_mod and extract the intercept
