...
ds<-read.csv("jonah_spring_2022.csv", header = TRUE)
...
p1 = ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'Jonah Crab') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("jonah_fall_2022.csv", header = TRUE)
...
intercept2 = 0.114466    # look at p2_mod and extract the intercept
