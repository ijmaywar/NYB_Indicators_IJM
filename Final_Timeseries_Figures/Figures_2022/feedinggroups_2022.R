...
Benthos<-read.csv("benthos_spring_2022.csv", header = TRUE)
Benthos
...
### Benthos Fall
Benthos_f<-read.csv("benthos_fall_2022.csv", header = TRUE)
Benthos_f
...
intercept_bf = 3.113969   # look at p2_mod and extract the intercept
pdata_bf <- transform(pdata_bf, p2_mod_bf = p2_mod_bf$fit[,1], se2 = p2_mod_bf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_bf.d <- Deriv(mod_bf, n=57) # n is the number of YEARs
...
Bt_f <- ggplot() + 
  ...
  geom_point(data = Benthos_f[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
Benthivore<-read.csv("benthivore_spring_2022.csv", header = TRUE)
Benthivore
...
### Benthivore Fall
Benthivore_f<-read.csv("Benthivore_fall_2022.csv", header = TRUE)
Benthivore_f
...
intercept_bvf = 19.43176   # look at p2_mod and extract the intercept
pdata_bvf <- transform(pdata_bvf, p2_mod_bvf = p2_mod_bvf$fit[,1], se2 = p2_mod_bvf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_bvf.d <- Deriv(mod_bvf, n=57) # n is the number of YEARs
...
Bv_f <- ggplot() + 
  ...
  geom_point(data = Benthivore_f[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
Planktivore<-read.csv("Planktivore_spring_2022.csv", header = TRUE)
Planktivore
...
### Planktivore Fall
Planktivore_f<-read.csv("Planktivore_fall_2022.csv", header = TRUE)
Planktivore_f
...
intercept_pvf = 25.68011   # look at p2_mod and extract the intercept
pdata_pvf <- transform(pdata_pvf, p2_mod_pvf = p2_mod_pvf$fit[,1], se2 = p2_mod_pvf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_pvf.d <- Deriv(mod_pvf, n=57) # n is the number of YEARs
...
Pv_f <- ggplot() + 
  ...
  geom_point(data = Planktivore_f[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
...
Piscivore<-read.csv("Piscivore_spring_2022.csv", header = TRUE)
Piscivore
...
### Piscivore Fall
Piscivore_f<-read.csv("Piscivore_fall_2022.csv", header = TRUE)
Piscivore_f
...
intercept_pcf = 70.63927    # look at p2_mod and extract the intercept
pdata_pcf <- transform(pdata_pcf, p2_mod_pcf = p2_mod_pcf$fit[,1], se2 = p2_mod_pcf$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod_pcf.d <- Deriv(mod_pcf, n=57) # n is the number of YEARs
...
Pc_f <- ggplot() + 
  ...
  geom_point(data = Piscivore_f[57,], aes(x= YEAR, y = strat.biomass), shape = 17, size =3) +
