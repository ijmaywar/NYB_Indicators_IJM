...
ds_n<-read.csv("north_spring_2022.csv", header = TRUE)
...
intercept = 26.42128      # look at p2_mod and extract the intercept
...
plot(ratio ~ YEAR, data = ds_n)
lines(ratio ~ YEAR, data = ds_n)
...
linearMod<- lm(ratio ~ YEAR, data=ds_n)
summary(linearMod)

p1 = ggplot() + 
  geom_line(data = ds_n, aes(x = YEAR, y = ratio), color = 'grey') +
  geom_point(data = ds_n, aes(x = YEAR, y = ratio), color = 'gray') + 
  geom_point(data = ds_n[54:55, ], aes(x = YEAR, y = ratio), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Ratio Northern:Southern"), x = '', title = 'Northern to Southern Species') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2_n<-read.csv("north_fall_2022.csv", header = TRUE)
ds2_s<-read.csv("south_fall_2022.csv", header = TRUE)
...
pdata2 <- with(ds2_n, data.frame(YEAR = YEAR))
p2_mod2 <- predict(mod2, newdata = pdata2,  type = "terms", se.fit = TRUE)
intercept2 = 5.786526     # look at p2_mod and extract the intercept
...
# Take a quick look to make sure it appears ok before final plotting
plot(ratio ~ YEAR, data = ds2_n)
lines(ratio ~ YEAR, data = ds2_n)
...
linearmod2<- lm(ratio ~ YEAR, data=ds2_n)
summary(linearmod2)

p2 = ggplot() + 
  geom_line(data = ds2_n, aes(x = YEAR, y = ratio), color = 'grey') +
  geom_point(data = ds2_n, aes(x = YEAR, y = ratio), color = 'gray') + 
  geom_point(data = ds2_n[57, ], aes(x = YEAR, y = ratio), shape = 17, size = 3) + 
  geom_smooth(data = ds2_n, aes(x = YEAR, y = ratio), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Ratio Northern:Southern"), x = 'Year', title = '') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
