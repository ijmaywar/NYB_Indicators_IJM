...
ds<-read.csv("bsb_spring_2022.csv", header = TRUE)
...
intercept = 0.3476082    # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  geom_point(data = ds[45:46, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'Black Sea Bass') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("bsb_fall_2022.csv", header = TRUE)
...
intercept2 = 0.1125761     # look at p2_mod and extract the intercept
pdata2 <- transform(pdata2, p2_mod2 = p2_mod2$fit[,1], se2 = p2_mod2$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod2.d <- Deriv(mod2, n=49) # n is the number of YEARs
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2[49, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
