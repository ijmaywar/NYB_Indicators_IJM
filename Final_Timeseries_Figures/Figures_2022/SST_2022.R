...
sst<-read.csv("OISST_Means_NOV_14_2022.csv", header = TRUE)
nyb = sst[sst$Loc == 'NYB',]
...
intercept_w = 7.709873 # look at p2_mod and extract the intercept
...
Term = "Year"
mod_w.d <- Deriv(mod_w, n=41) # n is the number of years
...
intercept_sp = 12.74018 # look at p2_mod and extract the intercept
pdata_sp <- transform(pdata_sp, p2_mod_sp = p2_mod_sp$fit[,1], se2_sp = p2_mod_sp$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "Year"
mod_sp.d <- Deriv(mod_sp, n=41) # n is the number of years
...
spr_plot <- ggplot() + 
  ...
  geom_point(data = spring[39:41,], aes(x= Year, y = Val), shape = 17, size =3) +
...
intercept_su = 22.37 # look at p2_mod and extract the intercept
pdata_su <- transform(pdata_su, p2_mod_su = p2_mod_su$fit[,1], se2_su = p2_mod_su$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "Year"
mod_su.d <- Deriv(mod_su, n=41) # n is the number of years
...
sum_plot <- ggplot() + 
  ...
  geom_point(data = summer[39:41,], aes(x= Year, y = Val), shape = 17, size =3) +
...
intercept_fa = 14.72729 # look at p2_mod and extract the intercept
pdata_fa <- transform(pdata_fa, p2_mod_fa = p2_mod_fa$fit[,1], se2_fa = p2_mod_fa$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "Year"
mod_fa.d <- Deriv(mod_fa, n=40) # n is the number of years
...
fall_plot <- ggplot() + 
  ...
  geom_point(data = fall[39:40,], aes(x= Year, y = Val), shape = 17, size =3) +
