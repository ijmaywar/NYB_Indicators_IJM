...
meanflow<-read.csv("Riverflow2021.csv", header = TRUE)
...
ggplot() + 
  geom_line(data = meanflow, aes(x = year, y = flowrate), color = 'grey') +
  geom_point(data = meanflow, aes(x = year, y = flowrate), color = 'gray') + 
