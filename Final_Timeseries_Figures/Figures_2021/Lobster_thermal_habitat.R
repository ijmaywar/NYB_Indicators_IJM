...
meanflow<-read.csv("Lobster_Thermal_Hab_Jan_6_2022.csv", header = TRUE)
...
mod<- gam(Percent_Inhospitable ~ s(X, k=5), data = meanflow)
summary(mod) #check out model
gam.check(mod)

pdata <- with(meanflow, data.frame(X = X))
p2_mod <- predict(mod, newdata = pdata,  type = "terms", se.fit = TRUE)
intercept = 0.07129839  # look at p2_mod and extract the intercept
...
plot( Percent_Inhospitable~ X, data = meanflow)
lines(Percent_Inhospitable ~ X, data = meanflow)
...
linearMod<- lm(Percent_Inhospitable ~ X, data=meanflow)
summary(linearMod)

ggplot() + 
  geom_line(data = meanflow, aes(x = X, y = Percent_Inhospitable), color = 'grey53') +
  geom_point(data = meanflow, aes(x = X, y = Percent_Inhospitable), color = 'gray53') + 
