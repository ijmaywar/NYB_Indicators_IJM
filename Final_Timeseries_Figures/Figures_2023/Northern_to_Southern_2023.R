...
ds_n<-read.csv("north_spring_2023.csv", header = TRUE)
...
pdata <- with(ds_n, data.frame(YEAR = YEAR))
...
ds2_n<-read.csv("north_fall_2023.csv", header = TRUE)
...
intercept2 = 5.69615    # look at p2_mod and extract the intercept
...
p2 = ggplot() + 
  ...
  geom_point(data = ds2_n[58, ], aes(x = YEAR, y = ratio), shape = 17, size = 3) + 
