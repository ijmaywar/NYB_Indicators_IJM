...
b_nyh = read.csv("Wind_buoynyh_Oct_15_2024.csv")
b_tt  = read.csv("Wind_buoytt_Oct_15_2024.csv")
b_isl = read.csv("Wind_buoyisl_Oct_15_2024.csv")
...
## Exclude some really early data from ISL Buoy

```{r}
b_isl <- b_isl[b_isl$year > 1990, ]
```
...
# Remove 2008 for NYH and 2009 for ISL because these are incomplete
view(df[c(which(df$year==2008 & df$buoy=='New York Harbor'),
  which(df$year==2009 & df$buoy=='South of Islip')),])

df <- all_warnings
view(df)

# Rename seasons
all_warnings$season <- as.factor(all_warnings$season)
levels(all_warnings$season) <- c("Fall", "Spring", "Summer", "Winter")
...
ggplot(all_warnings, aes(x=year, y=n_scw, color=buoy))+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5), plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face   = 'bold'), axis.text= element_text(color = 'black', size = 12))
...
# # Using the new labeller API
# variable_labeller2 <- function(df) {
#   if (df$variable == 'season') {
#     return(variable_names[df$value])
#   } else {
#     return(buoy_names)
#   }
# }
...
#   geom_line(stat='identity') +
#   facet_grid(season ~ buoy, scales = "free", space = "free_x", 
#              labeller = labeller(season = variable_labeller2, buoy = variable_labeller2)) +
#   theme_bw() +
#   labs(y = bquote("Number of Days"), x = '', title = 'Small Craft Warning Days') + 
...
# NY Harbor
```{r}
nyh_winter = all_warnings[which(all_warnings$buoy == 'New York Harbor' & all_warnings$season == 'winter'), ] 
nyh_spring = all_warnings[which(all_warnings$buoy == 'New York Harbor' & all_warnings$season == 'spring'), ] 
nyh_summer = all_warnings[which(all_warnings$buoy == 'New York Harbor' & all_warnings$season == 'summer'), ] 
nyh_fall = all_warnings[which(all_warnings$buoy == 'New York Harbor' & all_warnings$season == 'fall'), ] 

linearMod_w<- lm(n_scw ~ year, data=nyh_winter)
summary(linearMod_w) #no significant linear trend

qwinter = quantile(nyh_winter$n_scw, probs = c(.30,.70))
# find the last 5 years mean
mn_winter5 = mean(nyh_winter$n_scw[nyh_winter$year >= 2019])
# what quintile the data is in
mn_winter5 >qwinter #below avg for 2023 report


linearMod_sp<- lm(n_scw ~ year, data=nyh_spring)
summary(linearMod_sp) #no significant linear trend

qspring = quantile(nyh_spring$n_scw, probs = c(.3,.7))
# find the last 5 years mean
mn_spring5 = mean(nyh_spring$n_scw[nyh_spring$year >= 2019])
# what quintile the data is in
mn_spring5 >qspring  # above avg for 2023 report


linearMod_su<- lm(n_scw ~ year, data=nyh_summer)
summary(linearMod_su) #no significant linear trend

qsummer = quantile(nyh_summer$n_scw, probs = c(.3,.7))
# find the last 5 years mean
mn_summer5 = mean(nyh_summer$n_scw[nyh_summer$year >= 2019])
# what quintile the data is in
mn_summer5 >qsummer  # avg for 2023 report


linearMod_f<- lm(n_scw ~ year, data=nyh_fall)
summary(linearMod_f) #no significant linear trend

qfall = quantile(nyh_fall$n_scw, probs = c(.30,.70))
# find the last 5 years mean
mn_fall5 = mean(nyh_fall$n_scw[nyh_fall$year >= 2019])
# what quintile the data is in
mn_fall5 >qfall # avg for 2023 report
```

# South of Islip
```{r}
soi_winter = all_warnings[which(all_warnings$buoy == 'South of Islip' & all_warnings$season == 'winter'), ] 
soi_spring = all_warnings[which(all_warnings$buoy == 'South of Islip' & all_warnings$season == 'spring'), ] 
soi_summer = all_warnings[which(all_warnings$buoy == 'South of Islip' & all_warnings$season == 'summer'), ] 
soi_fall = all_warnings[which(all_warnings$buoy == 'South of Islip' & all_warnings$season == 'fall'), ] 

linearMod_w<- lm(n_scw ~ year, data=soi_winter)
summary(linearMod_w) #no significant linear trend

qwinter = quantile(soi_winter$n_scw, probs = c(.30,.70))
# find the last 5 years mean
mn_winter5 = mean(soi_winter$n_scw[soi_winter$year >= 2019])
# what quintile the data is in
mn_winter5 >qwinter #avg for 2023 report


linearMod_sp<- lm(n_scw ~ year, data=soi_spring)
summary(linearMod_sp) #no significant linear trend

qspring = quantile(soi_spring$n_scw, probs = c(.30,.70))
# find the last 5 years mean
mn_spring5 = mean(soi_spring$n_scw[soi_spring$year >= 2019])
# what quintile the data is in
mn_spring5 >qspring #avg for 2023 report


linearMod_su<- lm(n_scw ~ year, data=soi_summer)
summary(linearMod_su) #no significant linear trend

qsummer = quantile(soi_summer$n_scw, probs = c(.30,.70))
# find the last 5 years mean
mn_summer5 = mean(soi_summer$n_scw[soi_summer$year >= 2019])
# what quintile the data is in
mn_summer5 >qsummer #below avg for 2023 report


linearMod_f<- lm(n_scw ~ year, data=soi_fall)
summary(linearMod_f) #no significant linear trend

qfall = quantile(soi_fall$n_scw, probs = c(.3, 0.7))
# find the last 5 years mean
mn_fall5 = mean(soi_fall$n_scw[soi_fall$year >= 2019])
# what quintile the data is in
mn_fall5 >qfall #below avg for 2023 report
```

# Texas Tower
```{r}
tt_winter = all_warnings[which(all_warnings$buoy == 'Texas Tower' & all_warnings$season == 'winter'), ] 
tt_spring = all_warnings[which(all_warnings$buoy == 'Texas Tower' & all_warnings$season == 'spring'), ] 
tt_summer = all_warnings[which(all_warnings$buoy == 'Texas Tower' & all_warnings$season == 'summer'), ] 
tt_fall = all_warnings[which(all_warnings$buoy == 'Texas Tower' & all_warnings$season == 'fall'), ] 

linearMod_w<- lm(n_scw ~ year, data=tt_winter)
summary(linearMod_w) #no significant linear trend

qwinter = quantile(tt_winter$n_scw, probs = c(.30,.70))
# find the last 5 years mean
mn_winter5 = mean(tt_winter$n_scw[tt_winter$year >= 2019])
# what quintile the data is in
mn_winter5 >qwinter #below for 2023 report


linearMod_sp<- lm(n_scw ~ year, data=tt_spring)
summary(linearMod_sp) #significant linear trend

qspring = quantile(tt_spring$n_scw, probs = c(.3,.7))
# find the last 5 years mean
mn_spring5 = mean(tt_spring$n_scw[tt_spring$year >= 2019])
# what quintile the data is in
mn_spring5 >qspring #above avg for 2023 report


linearMod_su<- lm(n_scw ~ year, data=tt_summer)
summary(linearMod_su) #no significant linear trend

qsummer = quantile(tt_summer$n_scw, probs = c(.30,.7))
# find the last 5 years mean
mn_summer5 = mean(tt_summer$n_scw[tt_summer$year >= 2019])
# what quintile the data is in
mn_summer5 >qsummer #below avg for 2023 report


linearMod_f<- lm(n_scw ~ year, data=tt_fall)
summary(linearMod_f) #no significant linear trend

qfall = quantile(tt_fall$n_scw, probs = c(.3,0.7))
# find the last 5 years mean
mn_fall5 = mean(tt_fall$n_scw[tt_fall$year >= 2019])
# what quintile the data is in
mn_fall5 >qfall # above avg for 2023 report
```
