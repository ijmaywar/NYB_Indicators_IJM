...
ds<-read.csv("shortfin_spring_2023.csv", header = TRUE)
...
intercept = 0.06739435  # look at p2_mod and extract the intercept
...
Term = "YEAR"
mod.d <- Deriv(mod, n=54) # n is the number of YEARs
...
p1 = ggplot() + 
  ...
  #geom_point(data = ds[53:54, ], aes(x = YEAR, y = strat.biomass), shape = 17, size = 3) + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'Shortfin Squid') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("shortfin_fall_2023.csv", header = TRUE)
...
intercept2 = 1.562493   # look at p2_mod and extract the intercept
