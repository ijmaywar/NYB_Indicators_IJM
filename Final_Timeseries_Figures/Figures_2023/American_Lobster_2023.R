...
ds<-read.csv("amlob_spring_2023.csv", header = TRUE)
...
intercept = 0.4722018  # look at p2_mod and extract the intercept
...
p1 = ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Biomass"), x = '', title = 'American Lobster') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("amlob_fall_2023.csv", header = TRUE)
...
intercept2 = 0.8060788   # look at p2_mod and extract the intercept
