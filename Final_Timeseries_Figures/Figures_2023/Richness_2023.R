## Figures for Fish Species Richness
...
linearMod<- lm(n ~ YEAR, data=ds)
summary(linearMod)


p1 = ggplot() + 
  geom_line(data = ds, aes(x = YEAR, y = n), color = 'grey') +
  geom_point(data = ds, aes(x = YEAR, y = n), color = 'gray') + 
  #geom_point(data = ds[54:55, ], aes(x = YEAR, y = n), shape = 17, size = 3) + 
  geom_smooth(data = ds, aes(x = YEAR, y = n), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Number of Species"), x = '', title = 'Fish Species Richness') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("richness_fall_2023.csv", header = TRUE)


#calculate the 30th a and 70th percentiles for the short term column of the indicators report
qds2 = quantile(ds2$n, probs = c(.30, .70))

# find the last 5 years mean
mn_ds25 = mean(ds2$n[ds2$YEAR >= 2019])
# what quintile the data is in
mn_ds25 >qds2

# Creat a GAM - adjust k and remember to check model
mod2<- gam(n ~ s(YEAR, k=10), data = ds2)
...
intercept2 = 53.05172    # look at p2_mod and extract the intercept
...
# Take a quick look to make sure it appears ok before final plotting
plot(n ~ YEAR, data = ds2)
lines(n ~ YEAR, data = ds2)
...
linearmod2<- lm(n ~ YEAR, data=ds2)
summary(linearmod2)

p2 = ggplot() + 
  geom_line(data = ds2, aes(x = YEAR, y = n), color = 'grey') +
  geom_point(data = ds2, aes(x = YEAR, y = n), color = 'gray') + 
  geom_point(data = ds2[58, ], aes(x = YEAR, y = n), shape = 17, size = 3) + 
  geom_smooth(data = ds2, aes(x = YEAR, y = n), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Number of Species"), x = 'Year', title = '') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
