...
df<-read.csv("Lobster_Thermal_Hab_Jan_3_2023.csv", header = TRUE)

qdf = quantile(df$Percent_Inhospitable, probs = c(.3, 0.70))

# find the last 5 years mean
mn_df5 = mean(df$Percent_Inhospitable[df$X >= 2019])
...
intercept =   0.08429441 # look at p2_mod and eXtract the intercept
pdata <- transform(pdata, p2_mod = p2_mod$fit[,1], se2 = p2_mod$se.fit[,1])

Term = "X"
mod.d <- Deriv(mod, n=31) # n is the number of years
...
Percent_Inhospitable_plot <- ggplot() + 
  ...
  geom_point(data = df[29:31,], aes(x = X, y = Percent_Inhospitable), shape = 17, size =3) + 
