...
pop<-read.csv("Annual_Population_Estimates_for_New_York_State_and_Counties__Beginning_1970_20240109.csv", header = TRUE)
...
qpop = quantile(c_pop$pop, probs = c(.30, .70))

# find the last 5 years mean
mn_pop5 = mean(c_pop$pop[c_pop$Year >= 2019])
...
intercept = 7254526  # look at p2_mod and extract the intercept
...
ggplot() + 
  ...
  geom_point(data = c_pop[53,], aes(x = Year, y = pop), shape = 17, size =3) + 
