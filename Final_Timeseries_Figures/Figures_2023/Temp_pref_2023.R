## Figures for Temperature Preference of Fish Community
...
Term = "YEAR"
mod.d <- Deriv(mod, n=55) # n is the number of YEARs
...
plot(wt_mean_temp ~ YEAR, data = ds)
lines(wt_mean_temp ~ YEAR, data = ds)
...
linearMod<- lm(wt_mean_temp ~ YEAR, data=ds)
summary(linearMod)


p1 = ggplot() + 
  geom_line(data = ds, aes(x = YEAR, y = wt_mean_temp), color = 'grey') +
  geom_point(data = ds, aes(x = YEAR, y = wt_mean_temp), color = 'gray') + 
  #geom_point(data = ds[54:55, ], aes(x = YEAR, y = wt_mean_temp), shape = 17, size = 3) + 
  geom_smooth(data = ds, aes(x = YEAR, y = wt_mean_temp), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Mean Preferred Temp (\u00B0C)"), x = '', title = 'Temeperature Preference of the Fish Community') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("avgtemp_fall_2023.csv", header = TRUE)


#calculate the 30th a and 70th percentiles for the short term column of the indicators report
qds2 = quantile(ds2$wt_mean_temp, probs = c(.30, .70))

# find the last 5 years mean
mn_ds25 = mean(ds2$wt_mean_temp[ds2$YEAR >= 2019])
# what quintile the data is in
mn_ds25 >qds2

# Creat a GAM - adjust k and remember to check model
mod2<- gam(wt_mean_temp ~ s(YEAR, k=10), data = ds2)
...
intercept2 = 10.33753    # look at p2_mod and extract the intercept
pdata2 <- transform(pdata2, p2_mod2 = p2_mod2$fit[,1], se2 = p2_mod2$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "YEAR"
mod2.d <- Deriv(mod2, n=58) # n is the number of YEARs
...
# Take a quick look to make sure it appears ok before final plotting
plot(wt_mean_temp ~ YEAR, data = ds2)
lines(wt_mean_temp ~ YEAR, data = ds2)
...
linearmod2<- lm(wt_mean_temp ~ YEAR, data=ds2)
summary(linearmod2)

p2 = ggplot() + 
  geom_line(data = ds2, aes(x = YEAR, y = wt_mean_temp), color = 'grey') +
  geom_point(data = ds2, aes(x = YEAR, y = wt_mean_temp), color = 'gray') + 
  geom_point(data = ds2[58, ], aes(x = YEAR, y = wt_mean_temp), shape = 17, size = 3) + 
  geom_smooth(data = ds2, aes(x = YEAR, y = wt_mean_temp), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Mean Preferred Temp (\u00B0C)"), x = 'Year', title = '') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
