...
plot(TL ~ YEAR, data = ds)
lines(TL ~ YEAR, data = ds)
...
linearMod<- lm(TL ~ YEAR, data=ds)
summary(linearMod)


p1 = ggplot() + 
  geom_line(data = ds, aes(x = YEAR, y = TL), color = 'grey') +
  geom_point(data = ds, aes(x = YEAR, y = TL), color = 'gray') + 
  #geom_point(data = ds[54:55, ], aes(x = YEAR, y = TL), shape = 17, size = 3) + 
  #geom_smooth(data = ds, aes(x = YEAR, y = TL), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Trophic Level"), x = '', title = 'Average Trophic Level') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))


ds2<-read.csv("avgtl_fall_2023.csv", header = TRUE)



#calculate the 30th a and 70th percentiles for the short term column of the indicators report
qds2 = quantile(ds2$TL, probs = c(.30, .70))

# find the last 5 years mean
mn_ds25 = mean(ds2$TL[ds2$YEAR >= 2019])
# what quintile the data is in
mn_ds25 >qds2

# Creat a GAM - adjust k and remember to check model
mod2<- gam(TL ~ s(YEAR, k=10), data = ds2)
...
intercept2 = 3.930109    # look at p2_mod and extract the intercept
...
# Take a quick look to make sure it appears ok before final plotting
plot(TL ~ YEAR, data = ds2)
lines(TL ~ YEAR, data = ds2)
...
linearmod2<- lm(TL ~ YEAR, data=ds2)
summary(linearmod2)

p2 = ggplot() + 
  geom_line(data = ds2, aes(x = YEAR, y = TL), color = 'grey') +
  geom_point(data = ds2, aes(x = YEAR, y = TL), color = 'gray') + 
  geom_point(data = ds2[58, ], aes(x = YEAR, y = TL), shape = 17, size = 3) + 
  geom_smooth(data = ds2, aes(x = YEAR, y = TL), method = lm, se = FALSE, color = 'black') + 
  ...
  theme_bw() +
  labs (y = bquote("Trophic Level"), x = 'Year', title = '') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
