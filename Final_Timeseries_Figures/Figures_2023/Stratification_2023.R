## Figures for Stratification Seasonally
...
mn_winter5 = mean(winter$Val_I[winter$Year >= 2019])
...
wint_plot <- ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Density at 50m - Density at Surface"), x =' ',title = 'Winter') + 
...
# find the last 5 years mean
mn_spring5 = mean(spring$Val_I[spring$Year >= 2019])
# what quintile the data is in
mn_spring5 >qspring
...
intercept_sp = 0.001243506    # look at p2_mod and extract the intercept
...
# find the last 5 years mean
mn_summer5 = mean(summer$Val_I[summer$Year >= 2019])
# what quintile the data is in
mn_summer5 >qsummer

# Creat a GAM - adjust k and remember to check model
mod_su<- gam(Val_I ~ s(Year, k=5), data = summer)
...
intercept_su = 0.001449802  # look at p2_mod and extract the intercept
...
sum_plot <- ggplot() + 
  ...
  theme_bw() +
  labs (y = bquote("Density at 50m - Density at Surface"), x = 'Year', title = 'Summer') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
...
# find the last 5 years mean
mn_fall5 = mean(fall$Val_I[fall$Year >= 2019])
# what quintile the data is in
mn_fall5 >qfall
...
intercept_fa = 0.001043928   # look at p2_mod and extract the intercept
...
ggplot() + 
  ...
  geom_line(data=pdata_su, aes(x = Year, y = p2_mod_su+intercept_su), se = FALSE, color = 'black', linetype = 'twodash', size = 1) + 
  theme_bw() +
  labs (y = bquote("Density at surface/Density at 50m"), x = 'Year', title = 'Stratification') + 
  theme(plot.title=element_text(size = 16,face = 'bold',hjust = 0.5), axis.title=element_text(size = 14, face = 'bold'), axis.text= element_text(color = 'black', size = 12))
