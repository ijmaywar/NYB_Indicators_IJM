...
Landings<-read.csv("Commercial_Landings_2023.csv", header = TRUE)
Landings

#calculate the 30th a and 70th percentiles for the short term column of the indicators report
qds = quantile(Landings$KG, probs = c(.30, .70))

# find the last 5 years mean
mn_ds5 = mean(Landings$KG[Landings$Year >= 2019])
...
intercept =28775515  # look at p2_mod and extract the intercept
...
Term = "Year"
mod.d <- Deriv(mod, n=73) # n is the number of years
...
ggplot() + 
  ...
  geom_point(data = Landings[73,], aes(x=Year, y=KG),shape = 17, size =3) + 
...
#calculate the 30th a and 70th percentiles for the short term column of the indicators report
qds2 = quantile(Landings$DD, probs = c(.30, .70))

# find the last 5 years mean
mn_ds25 = mean(Landings$DD[Landings$Year >= 2019])
# what quintile the data is in
mn_ds25 >qds2
...
pdata <- with(Landings, data.frame(Year = Year))
p2_mod <- predict(mod, newdata = pdata,  type = "terms", se.fit = TRUE)
intercept = 27591822   # look at p2_mod and extract the intercept
pdata <- transform(pdata, p2_mod = p2_mod$fit[,1], se2 = p2_mod$se.fit[,1])
...
ggplot() + 
  ...
  geom_point(data = Landings[73,], aes(x=Year, y=DD),shape = 17, size =3) + 
