...
#transform NYB to common proj4string
NYBshp<-spTransform(NYBshp, EPUshp@proj4string)
Strata<-spTransform(Strata, EPUshp@proj4string)
#plot(EPUshp)
#plot(NYBshp, add = TRUE)
#plot(Strata, add = TRUE)

########################################################
#####THIS FUNCTION LABELS POINTS #######################
########################################################
klab<-function(LAT, LON){
#LAT = dat$lat
#LON = dat$lon

#make the points given into a spatial data frame
pts<-SpatialPointsDataFrame(data.frame(LON, LAT),
                            data = data.frame(rowID = 1:length(LAT),LAT,LON), proj4string = 
                              EPUshp@proj4string)

#DO the over function to create labels based on overlap
mabLab<-over(pts, EPUshp)
stratLab<-over(pts, Strata)
NYBshp$NYB<-"NYB"
nybLab<-over(pts, NYBshp)

###Add labels to pts
pts$EPU<-mabLab$EPU
pts$NYB<-nybLab$NYB
pts$Strata<-stratLab$FINSTR_ID

print(data.frame(EPU = pts$EPU,
           NYB = pts$NYB,
           STRATA = pts$Strata))
}
...
########################################################
#####THIS FUNCTION PLOTS GAM EFFECT OF s(LAT,LON)#######
########################################################
