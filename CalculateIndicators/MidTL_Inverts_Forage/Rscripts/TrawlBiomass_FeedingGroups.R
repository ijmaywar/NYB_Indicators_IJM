...
#Calculate mean CPUE of fish groups according to NOAA 2018 version
...
fin<-aggregate(MeanSP ~ SOE_18 + EST_YEAR + SEASON, ccc, FUN = sum, drop = FALSE)
...
firstDF<-data.frame(Year = fin$EST_YEAR,
                    Variable = paste(fin$SEASON, fin$SOE_18, sep = "_"),
                    Val = fin$MeanSP,
                    Loc = "NES",
                    N = fin$Towperyear)

plot(firstDF[firstDF$Variable == "SPRING_Benthivore","Year"],
     firstDF[firstDF$Variable == "SPRING_Benthivore","Val"], type = "b")
points(firstDF[firstDF$Variable == "FALL_Benthivore","Year"],
     firstDF[firstDF$Variable == "FALL_Benthivore","Val"], type = "b")
...
for(i in 48:49){
  ...
  #here it is important to use drop = FALSE because it then puts NAs for where zeros should be
  fin<-aggregate(MeanSP ~ SOE_18 + EST_YEAR + SEASON + EPU, ccc, FUN = sum, drop = FALSE)
  fin$MeanSP[is.na(fin$MeanSP)]<-0#these are REAL zeros
...
firstDF<-firstDF[firstDF$Variable %in% c("FALL_Apex Predator", "SPRING_Apex Predator") == FALSE,]
setwd("~/Desktop/NYB Indicators/Final_timeseries")
write.csv(firstDF, "Fish_Groups_AUG24.csv")
