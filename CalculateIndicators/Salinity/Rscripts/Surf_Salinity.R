#Purpose: Create Salinity Indicator (Surface)
...
NYB_m1<-gam(surf_sal ~ s(lon,lat, k = 100) + s(month) +  year_fac, 
        data = subdat1)
...
MAB_m1<-gam(surf_sal ~ s(lon,lat, k = 100) + s(month) +  year_fac, 
            data = subdat1)
...
######DO THE ANALYSIS#############
NES_m1<-gam(surf_sal ~ s(lon,lat, k = 100) + s(month) +  year_fac, 
            data = subdat1)
