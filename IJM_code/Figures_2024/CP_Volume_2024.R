...
# cp_raw <- read.csv("CP_volume_Nov_26_2024.csv", header = TRUE)
cp_raw <- read.csv("CP_volume_Dec_19_2024.csv", header = TRUE)
cp <- cp_raw
cp[, colnames(cp) != "X"] <- cp[, colnames(cp) != "X"] /(1000*1000*1000)
...
mn_june5 = mean(cp$June[cp$X %in% tail(unique(cp$X))])
...
intercept_w = mod_june$coefficients[1]   # look at p2_mod and extract the intercept
...
Term = "X"
mod_june.d <- Deriv(mod_june, n=length(unique(mod_june$model$X))) # n is the number of Xs
...
june_plot <- ggplot() + 
  ...
  xlim(1993,2024) +
  geom_text(
    x = latest_june$X, y = latest_june$June,
    aes(label = latest_june$X),  # Custom label
...
# find the last 5 years mean
mn_July5 = mean(cp$July[cp$X %in% tail(sort(cp$X))])
# what quintile the data is in
mn_July5 < qJuly
...
intercept_july = mod_july$coefficients[[1]]    # look at p2_mod and extract the intercept
pdata_july <- transform(pdata_july, p2_mod_july = p2_mod_july$fit[,1], se2_july = p2_mod_july$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "X"
mod_july.d <- Deriv(mod_july, n=length(unique(mod_july$model$X))) # n is the number of Xs
...
july_plot <- ggplot() + 
  ...
  xlim(1993,2024) +
  geom_text(
    x = latest_july$X, y = latest_july$July,
    aes(label = latest_july$X),  # Custom label
...
# find the last 5 years mean
mn_August5 = mean(cp$August[cp$X %in% tail(sort(cp$X))])
# what quintile the data is in
mn_August5 < qAugust
...
intercept_aug = mod_aug$coefficients[[1]]  # look at p2_mod and extract the intercept
pdata_aug <- transform(pdata_aug, p2_mod_aug = p2_mod_aug$fit[,1], se2_aug = p2_mod_aug$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "X"
mod_aug.d <- Deriv(mod_aug, n=length(unique(mod_aug$model$X))) # n is the number of Xs
...
aug_plot <- ggplot() + 
  ...
  xlim(1993,2024) +
  geom_text(
    x = latest_aug$X, y = latest_aug$August,
    aes(label = latest_aug$X),  # Custom label
    hjust = 1, vjust = -1,             # Adjust position
    color = "black", size = 4              # Styling
  ) +
...
intercept_sep =  mod_sep$coefficients[[1]]
pdata_sep <- transform(pdata_sep, p2_mod_sep = p2_mod_sep$fit[,1], se2_sep = p2_mod_sep$se.fit[,1])

#  Now that we have the model prediction, the next step is to calculate the first derivative
#  Then determine which increases and decreases are significant
Term = "X"
mod_sep.d <- Deriv(mod_sep, n=length(unique(mod_sep$model$X))) # n is the number of Xs
...
sep_plot <- ggplot() + 
  ...
  xlim(1993,2024) +
  geom_text(
    x = latest_sep$X, y = latest_sep$September,
    aes(label = latest_sep$X),  # Custom label
    hjust = 1, vjust = -1,             # Adjust position
    color = "black", size = 4              # Styling
  ) +
