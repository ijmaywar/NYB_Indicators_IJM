...
nyb$Val_I <- exp(nyb$Intercept + nyb$Val)
...
mn_winter5 = mean(winter$Val_I[winter$Year %in% tail(sort(unique(winter$Year)))])
...
# find the last 5 years mean
mn_spring5 = mean(spring$Val_I[spring$Year %in% tail(sort(unique(spring$Year)))])
# what quintile the data is in
mn_spring5 > qspring
...
spr_plot <- ggplot() + 
  ...
  geom_text(
    ...
    hjust = .4, vjust = 2,              # Adjust position
    color = "black", size = 4              # Styling
  ) +
...
# find the last 5 years mean
mn_summer5 = mean(summer$Val_I[summer$Year %in% tail(sort(summer$Year))])
# what quintile the data is in
mn_summer5 >qsummer
...
sum_plot <- ggplot() + 
  ...
  geom_point(data = summer[41,], aes(x= Year, y = Val_I), shape = 17, size =3) +
...
# find the last 5 years mean
mn_fall5 = mean(fall$Val_I[fall$Year %in% tail(sort(fall$Year))])
# what quintile the data is in
mn_fall5 >qfall
...
fall_plot <- ggplot() + 
  ...
  geom_point(data = fall[35,], aes(x= Year, y = Val_I), shape = 17, size =3) +
  ...
  geom_text(
    ...
    hjust = .2, vjust = -1,              # Adjust position
    color = "black", size = 4              # Styling
  ) +
