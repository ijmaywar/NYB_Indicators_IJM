...
setwd("/Users/ian/Desktop/*NYB Indicators/CalculateIndicators/WaterTemperature/Data/WOD/GLD")
files <- list.files(pattern = "\\.nc$")
file_cons <- list()

for (file in files) { 
  file_cons[[file]] <- ncdf4::nc_open(file)    
}

###heres just a look at the dataset, a map with points colored according to botom depth
cfunk<-colorRampPalette(c("red","blue"))
col = cfunk(100)[as.numeric(cut(as.vector(ncdf4::ncvar_get(temp, "Bottom_Depth")), breaks = 100))]

plot(as.vector(ncdf4::ncvar_get(temp, "lon")),as.vector(ncdf4::ncvar_get(temp, "lat")),
...
ctds_list <- list()
ddd_con_list <- list()
for (j in 1:length(file_cons)) {
  ddd<-make_casts(file_cons[[j]])
  ctds1<-list()#this is the list that will contain individual CTD casts  
  z<-as.vector(ncdf4::ncvar_get(file, "z"))#get vectors of variables
  t<-as.vector(ncdf4::ncvar_get(file, "Temperature"))#get vectors of variables
  
  for(i in 1:nrow(ddd)){
    ctds1[[i]]<-create.ctd(i)
  }
  ctds_list[[j]] <- ctds1
  ddd_con_list[[j]] <- ddd #resave ddd as ddd1 
  ###Now you have ctds1 (has a list of ctd data) and ddd_con_list[[1]] (has the cast info)
}
```
...
plot(ctds_list[[2]][[i]]$temp, ctds_list[[2]][[i]]$depth, ylim =c(rev(range(ctds_list[[2]][[i]]$depth))),
...
for (j in 1:length(ddd_con_list)) {
  nrow(ddd_con_list[[j]])#
  length(ctds_list[[j]])#good same length
  ddd_con_list[[j]]$last_row_depth<-NA#depth of last row in cast i
  ddd_con_list[[j]]$bot_temp<-NA#the temperature of the last row in cast i
  
  ddd_con_list[[j]]$first_row_depth<-NA#depth of first row in cast i
  ddd_con_list[[j]]$surf_temp<-NA#temp of first row in cast i
  
  ddd_con_list[[j]]$c50_depth<-NA#depth of obs closest to 50 m
  ddd_con_list[[j]]$c50_temp<-NA#temp of obs closst to 50 m
  
  for(i in 1:nrow(ddd_con_list[[j]])){
    thecast<-ctds_list[[j]][[i]]#the cast data
    c50row<-which.min(abs(thecast$depth - 50))#row index of obs clsoest to 50 m
    
    ddd_con_list[[j]][i,"last_row_depth"]<-thecast[nrow(thecast),"depth"]
    ddd_con_list[[j]][i,"bot_temp"]<-thecast[nrow(thecast),"temp"]
    
    ddd_con_list[[j]][i,"first_row_depth"]<-thecast[1,"depth"]
    ddd_con_list[[j]][i,"surf_temp"]<-thecast[1,"temp"]
   
    ddd_con_list[[j]][i, "c50_depth"]<-thecast[c50row, "depth"] # ERROR IS HERE - thecast is all NA
    ddd_con_list[[j]][i, "c50_temp"]<-thecast[c50row, "temp"]
  
    # print(i)
  }
}
#Now ddd_con_list has what you want

for (j in 1:length(ddd_con_list)) {
   
}
finalDDD<-rbind(ddd_con_list[[1]],ddd_con_list[[2]])
```
