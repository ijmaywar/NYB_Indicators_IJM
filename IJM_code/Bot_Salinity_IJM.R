...
# Create Salinity Indicator (Bottom) from WOD and seawolf CTDs
#
...
for (szn in c("winter","spring","summer","fall")) {
  ...
  NYB_m1<-gam(bot_sal ~ s(lon,lat, k = 100) + s(month,k=3) +  year_fac, 
              data = subdat1)
  ...
  MAB_m1<-gam(bot_sal ~ s(lon,lat, k = 100) + s(month,k=3) +  year_fac, 
              data = subdat1)
  ...
  ######DO THE ANALYSIS#############
  NES_m1<-gam(bot_sal ~ s(lon,lat, k = 100) + s(month,k=3) +  year_fac, 
              data = subdat1)
...
write.csv(fff, "BotSal_insitu_Nov_17_2024.csv")
