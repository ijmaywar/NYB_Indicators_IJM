...
#  Then determine which increases and decreases are significant
Term = "Year"
mod_fa.d <- Deriv(mod_fa, n=length(unique(fall$Year))) # n is the number of years
...
fall_plot <- ggplot() + 
  ...
  geom_point(data = fall[length(unique(fall$Year)),], aes(x= Year, y = Val), shape = 17, size =3) +
